...
const App: React.FC = () => {
  ...
  const fetchEvents = async () => {
    ...
    // Calculate the range: 1 month back and 1 month forward
    const today = new Date();

    const oneMonthAgo = new Date(today);
    oneMonthAgo.setDate(1); // Set to the 1st to avoid overflow issues
    oneMonthAgo.setMonth(today.getMonth() - 1);

    const oneMonthFromNow = new Date(today);
    oneMonthFromNow.setDate(1); // Set to the 1st to avoid overflow issues
      ...
      {user ? (
        <div className="w-full max-w-md space-y-4">
          ...
          <div className="w-full">
            ...
            <button
              onClick={fetchEvents}
              className="w-full p-2 my-2 bg-green-500 text-white rounded-md hover:bg-green-600"
            >
              Fetch All Events
            </button>
